...
# when you import this file as-is, will still work because the statement running main below will no longer be true, so the main function is only here for testing purposes
...
class database:
    ...
    # allows object to be used as an iterator, by using yield
    def __iter__(self):
        cursor = self._db.execute('select * from {} order by t1'.format(self._table))
        for row in cursor:
            yield dict(row)

    # allows filename to be assigned, connects to db and sets up row factory
    @property
    ...
    # when delete filename it closes the db
    @filename.deleter
