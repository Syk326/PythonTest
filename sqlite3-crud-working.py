...
# individual db tasks broken down in a simple way
# actual production would be more OO

def insert(db, row):
    # takes string of SQL, ? = placeholders for tuple
    db.execute('insert into test (t1, i1) values (?, ?)', (row['t1'], row['i1']))
    # call commit everytime something can change the database, otherwise return or print or do something else
    db.commit()

def retrieve(db, t1):
    cursor = db.execute('select * from test where t1 = ?', (t1,))
    # returns object that can be used as dict?
...
def main():
    ...
    print('Create rows')
    # insert function defined above, not 'create' because not creating table/db
    ...
    print('Retrieve rows')
    # passes in a key and gets row object in return
